...
select category,min(order_date) as first_order ,max(order_date) as latest_order,sum(profit) as profit from orders
group by category;

--4- write a query to find sub-categories where average profit is more than the half of the max profit in that sub-category
select sub_category,avg(profit) as avg_profit,max(profit) max_profit from orders
...
select student_id,count(marks) from exams
where subject in('Chemistry','Physics')
group by marks,student_id
having count(marks) =2;

--6- write a query to find total number of products in each category.
select category,count(distinct product_name)as product_count from orders
group by category;

--7- write a query to find top 5 sub categories in west region by total quantity sold
select top 5 sub_category,sum(quantity) as quantity from orders
where region = 'West'
group by sub_category
order by quantity desc;

--8- write a query to find total sales for each region and ship mode combination for orders in year 2020
select region,ship_mode,sum(sales) as sales from orders
