...
select e1.emp_name,e2.emp_name as manager_name,abs(DATEDIFF(day,e1.dob,e2.dob)) AS DateDiff from employee e1
join employee e2
...
where DATENAME(month,o.order_date) = 'november'
...
select e1.emp_name as manager,STRING_AGG(e2.emp_name ,',' )  WITHIN GROUP (ORDER BY e2.salary) as list_of_employees from employee e1
...
select order_id,datename(WEEKDAY,order_date) as order_day,datename(WEEKDAY,ship_date) ship_day ,datediff(DAY,order_date,ship_date) as business_days
 ,datediff(week,order_date,ship_date) week_days from orders
 where DATENAME(WEEKDAY,order_date) not in ('saturday','sunday') and DATENAME(WEEKDAY,ship_date) not in ('saturday','sunday')
...
select o.category,sum(o.sales) total_sales,
case 
when r.return_reason is not null then sum(o.sales)
...
select o.category,sum(o.sales) total_sales,
sum(case 
when r.return_reason is not null then o.sales end )as total_return_sales
from orders o 
left join returns r 
on o.order_id=r.order_id
group by o.category;
...
--category, (sales in year 2019), total_sales_2020(sales in year 2020)
select category,
sum(case
when datepart(yy,order_date) = 2019 then sales  
end )as total_sales_2019,
sum(case
when datepart(yy,order_date) = 2020 then sales 
...
select top 5  city, avg(datediff(day,order_date,ship_date)) as avg_diff_day from orders
where region = 'West'
group by city
order by avg_diff_day desc;
...
select e1.emp_name , e2.emp_name as manager,e3.emp_name as super_manager from employee e1
inner join employee e2
on e1.manager_id = e2.emp_id
